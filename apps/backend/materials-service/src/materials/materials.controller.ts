...
export class MaterialsController {
  ...
  @Get(':id/suppliers')
  async getSupplierSuggestions(@Param('id') id: string) {
    return this.intelligentRecommendationService.suggestSuppliers(id);
  }
...
export class MaterialsController {
  ...
  async getStockPrediction(@Param('id') id: string) {
    ...
    if (this.mlTrainingService.hasHistoricalData(id)) {
      const mlPrediction = await this.mlTrainingService.predictStock(id, 24, material.quantity, material.reorderPoint);
      mlPrediction.materialName = material.name;
      mlPrediction.currentStock = material.quantity;
      return mlPrediction;
    }
  ...
  async scanQRCode(@UploadedFile() file: Express.Multer.File): Promise<QRScanResult> {
    ...
    try {
      ...
      let material: Material | null = null;
      if (parsedData.id) {
        try { material = await this.materialsService.findOne(parsedData.id); } catch(e) {}
      }
      if (!material && parsedData.code) {
        try { material = await this.materialsService.findByCode(parsedData.code); } catch(e) {}
      }
      if (!material) {
        try { material = await this.materialsService.findByQRCode(qrData); } catch(e) {}
      }
      if (fs.existsSync(file.path)) fs.unlinkSync(file.path);
      return { success: true, qrData, material };
    } catch (error) {
  ...
  @Post('scan-qr-text')
  async scanQRCodeText(@Body() scanDto: ScanQRDto): Promise<QRScanResult> {
    if (!scanDto.qrCode) throw new BadRequestException('QR code text requis');
    const parsedData = QRScannerUtil.parseQRData(scanDto.qrCode);
    let material: Material | null = null;
    if (parsedData.id) {
      try { material = await this.materialsService.findOne(parsedData.id); } catch(e) {}
    ...
    if (!material) {
      try { material = await this.materialsService.findByQRCode(scanDto.qrCode); } catch(e) {}
    }
    return { success: true, qrData: scanDto.qrCode, material };
  }
  ...
  async exportToExcel(@Res() res: Response, @Body() materialIds?: string[]) {
    const result = await this.materialsService.exportToExcel(materialIds);
    if (!result || !result.filePath) throw new BadRequestException('Erreur export Excel');
    return res.download(result.filePath, result.filename);
  }
  ...
  async exportToPDF(@Res() res: Response, @Body() materialIds?: string[]) {
    try {
      const result = await this.materialsService.exportToPDF(materialIds);
      if (!result || !result.filePath) throw new BadRequestException('Erreur génération PDF');
      return res.download(result.filePath, result.filename);
    } catch (error) {
      throw new BadRequestException(error.message);
    }
  ...
  @Post(':id/upload-csv')
  @UseInterceptors(FileInterceptor('file'))
  async uploadHistoricalData(@Param('id') id: string, @UploadedFile() file: Express.Multer.File) {
    try {
      if (!file) throw new BadRequestException('Aucun fichier CSV uploaded');
      const material = await this.materialsService.findOne(id);
      if (!material) throw new BadRequestException('Matériau non trouvé');
      let csvContent: string;
      if (file.buffer && file.buffer.length > 0) {
        csvContent = file.buffer.toString('utf-8');
      } else if (file.path) {
  ...
  async trainModel(@Param('id') id: string) {
    ...
    if (!material) throw new BadRequestException('Matériau non trouvé');
    const hasData = this.mlTrainingService.hasHistoricalData(id);
    if (!hasData) throw new BadRequestException('Aucune donnée historique. Upload CSV first.');
    const result = await this.mlTrainingService.trainModel(id, material.name, material.quantity, material.reorderPoint);
    return {
      success: true,
      message: `Model trained successfully! Accuracy: ${(result.accuracy * 100).toFixed(1)}%`,
      trainingResult: result,
  ...
  @Get(':id/predict')
  async predictStock(@Param('id') id: string, @Query('hours') hours: string = '24') {
    const material = await this.materialsService.findOne(id);
    if (!material) throw new BadRequestException('Matériau non trouvé');
    const hoursAhead = parseInt(hours, 10) || 24;
    const prediction = await this.mlTrainingService.predictStock(id, hoursAhead, material.quantity, material.reorderPoint);
    prediction.materialName = material.name;
    return prediction;
  }
  ...
  @Post(':id/predict-advanced')
  async predictAdvanced(@Param('id') id: string, @Body() features: { hourOfDay: number; dayOfWeek: number; siteActivityLevel: number; weather: string; projectType: string }) {
    const material = await this.materialsService.findOne(id);
    if (!material) throw new BadRequestException('Matériau non trouvé');
    const prediction = await this.mlTrainingService.predictStockAdvanced(id, {
      hourOfDay: features.hourOfDay,
      dayOfWeek: features.dayOfWeek,
      siteActivityLevel: features.siteActivityLevel,
      weather: features.weather,
  ...
  // ========== SMART SCORE ENDPOINTS ==========

  @Post('smart-score/site')
  async calculateSiteSmartScore(@Body() body: { siteId: string; siteName: string; progress: number }) {
    return this.smartScoreService.calculateSiteHealthScore(body.siteId, body.siteName, body.progress);
  }

  @Post('smart-score/sites')
  async calculateMultipleSitesScores(@Body() body: { sites: Array<{ id: string; name: string; progress: number }> }) {
    return this.smartScoreService.getAllSitesScores(body.sites);
  }

  @Get('smart-score/material/:materialId')
  async getMaterialScores(@Param('materialId') materialId: string) {
    const material = await this.materialsService.findOne(materialId);
    return {
      materialId: material._id,
      materialName: material.name,
      consumptionScore: material.consumptionScore || 0,
      stockHealthScore: material.stockHealthScore || 0,
      anomaliesScore: material.anomaliesScore || 0,
      siteHealthScore: material.siteHealthScore || 0,
      lastScoreUpdate: material.lastScoreUpdate,
    };
  }

  @Get('smart-score/site/:siteId/materials')
  async getSiteMaterialsWithScores(@Param('siteId') siteId: string) {
    const { Types } = require('mongoose');
    const materials = await this.materialsService['materialModel'].find({
      $or: [
    ...
    return materials.map(m => ({
      ...
      quantity: m.quantity,
      unit: m.unit,
      consumptionScore: m.consumptionScore || 0,
      stockHealthScore: m.stockHealthScore || 0,
      anomaliesScore: m.anomaliesScore || 0,
      siteHealthScore: m.siteHealthScore || 0,
    }));
  }
}
