...
export class PaymentService {
  ...
  private readonly PAYMENT_API_URL = process.env.PAYMENT_API_URL || 'http://localhost:3007/api/payments';
  private readonly FACTURE_API_URL = process.env.FACTURE_API_URL || 'http://localhost:3007/api/factures';
  ...
  private async createStripePaymentIntent(amount: number): Promise<string> {
    try {
      const stripeApiUrl = process.env.STRIPE_API_URL || 'http://localhost:3007/api/payments/stripe/create-payment-intent';
