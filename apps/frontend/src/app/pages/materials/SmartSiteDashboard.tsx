...
siteApiClient.interceptors.request.use((config) => {
...
const materialService = {
  ...
  ): Promise<{ data: Material[]; total: number; page: number; totalPages: number } | Material[]> {
    try {
  ...
  async bulkCreate(materials: CreateMaterialData[]): Promise<Material[]> {
    try {
      const response = await apiClient.post('/bulk', materials);
      return response.data;
    } catch (error) {
  ...
  async exportToExcel(materialIds?: string[]): Promise<Blob> {
    try {
      const response = await apiClient.post('/export/excel', materialIds || [], {
        responseType: 'blob',
      });
      return response.data;
    } catch (error) {
  ...
  async exportToPDF(materialIds?: string[]): Promise<Blob> {
    try {
      const response = await apiClient.post('/export/pdf', materialIds || [], {
        responseType: 'blob',
      });
      return response.data;
    } catch (error) {
