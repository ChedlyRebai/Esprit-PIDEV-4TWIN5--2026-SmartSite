...
import { updateCatalogItem, getCatalogItemById, CatalogItem } from "@/app/action/catalog.action";
...
export default function EditCatalogItem() {
  ...
  useEffect(() => {
    if (!canManageCatalog) {
      toast.error("You don't have permission to edit materials");
      navigate("/catalog");
      return;
    }
    loadData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await updateCatalogItem(id!, formData);
      if (res.status === 200) {
